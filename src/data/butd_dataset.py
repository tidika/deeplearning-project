"""Dataset and DataLoader for pre-extracted BUTD region features.

Expects features pre-extracted by scripts/extract_butd_features.py and
saved as:
...
class BUTDCaptionDataset(Dataset):
    """One sample per (image, caption) pair, using pre-extracted region features.

    For each image_id, loads the corresponding .pt file from features_dir.
    Images without a feature file are silently skipped.
    """
    ...
    def __init__(
        ...
        if missing:
            print(
                f"[BUTDCaptionDataset] {missing}/{len(split_image_ids)} images have no "
                "feature file and were skipped. Run scripts/extract_butd_features.py "
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        # Load pre-extracted region features: (num_regions, feature_dim)
        feat_path = self.features_dir / f"{image_id}.pt"
...
def _collate_butd(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    features = torch.stack([s["features"] for s in batch])  # (B, K, D)
...
def create_butd_dataloader(
    ...
    """Build a DataLoader over pre-extracted BUTD features for one split."""
    dataset = BUTDCaptionDataset(
