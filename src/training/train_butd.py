"""Training and evaluation utilities for the BUTD captioning model.

Provides three importable functions used by notebooks/train_butd.ipynb:
    train_one_epoch  — one pass over the training DataLoader
    evaluate_loss    — validation loss over the val DataLoader
    generate_captions — greedy decoding over a DataLoader, returns
...
def train_one_epoch(
    ...
    Args:
        model:     BUTDCaptionModel (moved to device by the caller).
        loader:    DataLoader from create_butd_dataloader.
        optimizer: Any torch optimizer.
        criterion: Loss function; typically CrossEntropyLoss(ignore_index=0).
        device:    Compute device.
        grad_clip: Max gradient norm (clips LSTM weights for stability).
    ...
    for batch in pbar:
        features = batch["features"].to(device)       # (B, K, feature_dim)
        caption_ids = batch["caption_ids"].to(device)  # (B, T) input tokens
        target_ids = batch["target_ids"].to(device)    # (B, T) shifted targets

        # Forward: feed caption_ids as inputs (teacher forcing)
...
def generate_captions(
    ...
    for batch in loader:
        ...
        # Skip images already processed (avoid re-generating for duplicate entries)
